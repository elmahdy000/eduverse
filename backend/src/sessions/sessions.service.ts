...
export class SessionsService {
    ...
    const session = await this.prisma.$transaction(
      async (tx) => {
        ...
        if (createSessionDto.roomId) {
          ...
          if (
            customer.customerType === 'trainer' &&
            activeRoomSessions.length > 0
          ) {
            throw new ConflictException(
              'Cannot reserve room for trainer while other active sessions exist',
            );
          }
        }
        ...
        if (
          !activeSubId &&
          (createSessionDto.sessionType === 'package' ||
            createSessionDto.billingType === 'subscription_covered')
        ) {
        ...
        const newSession = await tx.session.create({
          data: {
            customerId: createSessionDto.customerId,
            sessionType: createSessionDto.bookingId
              ? 'booking_linked'
              : createSessionDto.sessionType,
            billingType:
              createSessionDto.billingType ||
              (activeSubId ? 'subscription_covered' : 'hourly_individual'),
  ...
  async closeSession(
    ...
    this.logger.log(
      'Attempting to close session ' + sessionId + ' by user ' + userId,
    );
    ...
    if (!session) {
      this.logger.warn(
        'Failed to close session ' + sessionId + ': session not found',
      );
    ...
    if (!openShift) {
      this.logger.warn(
        'Failed to close session ' +
          sessionId +
          ': user does not have an active open shift',
      );
    ...
    if (session.chargeAmount !== null && session.chargeAmount !== undefined) {
      ...
      session.sessionType === 'package' ||
      session.billingType === 'subscription_covered'
    ) {
      chargeAmount = 0;
      ...
      const roomHourly = Number(
        session.room?.wholeRoomHourlyRate ?? session.room?.hourlyRate ?? 50,
      );
      chargeAmount = hours * roomHourly;
    } else if (
      session.sessionType === 'hourly' ||
      session.billingType === 'hourly_individual'
    ) {
      let rate: number;
      if (session.room) {
      ...
      if (session.room) {
        ...
        chargeAmount = coworkingRoom?.dailyRate
          ? Number(coworkingRoom.dailyRate)
          : 150;
      }
    } else {
      chargeAmount = 0;
    ...
    try {
      const closedSession = await this.prisma.$transaction(async (tx) => {
        ...
        this.logger.log(
          'Transaction successful: Session ' +
            sessionId +
            ' closed and invoice ' +
            invoice.id +
            ' created',
